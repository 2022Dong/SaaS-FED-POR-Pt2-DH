        ...
        <section class="px-4 pb-8">
            <header class="flex flex-row justify-between items-center gap-2">
                ...
                <section class="flex flex-row justify-between gap-4">
                    <a href="{{ route('users.index') }}" class="p-2 px-4 text-center rounded-md h-10 flex gap-4
                              text-gray-600 hover:text-gray-200 bg-gray-200 hover:bg-gray-500
                              duration-300 ease-in-out transition-all items-center">
                        <i class="fa fa-users text-lg"></i>
                        {{ __('Users') }}
                    </a>
                    <form class="flex flex-row gap-2 items-center justify-end" action="{{ route('users.trash-recover') }}" method="POST">
                        @CSRF
                    ...
                    <form class="flex flex-row gap-2 items-center justify-end" action="{{ route('users.trash-empty') }}" method="POST">
                        @CSRF
                        @method('delete')
                        <button type="submit" class="p-2 px-4  text-center rounded-md
                ...
                <tbody>
                    ...
                    <tr class="group
                        ...
                        <td class="py-2 pr-2 text-right">
                            <form class="flex flex-row gap-2 items-center justify-end" action="{{ route('users.trash-remove', $user) }}" method="POST">
                                @csrf
                                @method('delete')

                                <a href="{{ route('users.trash-restore', $user) }}" class="py-1 px-4 text-center rounded-md flex gap-4 items-center
