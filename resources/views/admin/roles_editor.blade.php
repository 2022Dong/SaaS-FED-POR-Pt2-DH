            ...
            <section class="w-full md:w-1/3 border shadow rounded
                ...
                <div class="flex flex-col gap-0 mb-4 flex-wrap">
                    ...
                    <form class="flex flex-row align-middle mb-2 border-b p-4
                                       border-neutral-300 " role="form" method="POST" action="{{ route('admin.assign-role') }}">
                        ...
                        <div class="flex flex-row gap-4 items-center">
                            <label class="text-neutral-700 leading-normal p-0 grow" for="role-id-{{ $role->id }}">Add:</label>
                            ...
                            <button type="submit" class="align-middle text-center leading-normal select-none
                    ...
                    <form class="flex flex-row items-center
                                             ...
                                             px-4 py-1
                                             group transition-all duration-500 ease-in-out" role="form" method="POST" action="{{ route('admin.revoke-role') }}">
                        ...
                        <a href="{{ route('users.show', $user->id ) }}" class="px-4 grow text-neutral-900
                                              transition-all duration-500 ease-in-out">
                            {{ $user->name }}
                        </a>
                        @can('role-revoke')
                        <button type="submit" class="align-middle text-center select-none
                                                  rounded
                                                  py-1 px-4
                                                  ...
                                                  transition-all duration-500 ease-in-out
                                                  print:hidden" value=" X " title="Revoke">
                            <i class="fa fa-times text-lg" title="Revoke Role for User {{$user->name}}"></i>
                        </button>
                        @endcan
                    </form>
                    @else
                    <a href="{{ route('users.show', $user->id ) }}" class="px-4 grow text-neutral-600 hover:text-neutral-200">
                        {{ $user->name }}
                    </a>
                    @endif
                    @endforeach
